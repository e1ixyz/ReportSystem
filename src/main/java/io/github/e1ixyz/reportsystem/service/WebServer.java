...
public class WebServer {
    ...
    private boolean isOpen(String path) {
        ...
        List<String> openPaths = cfg.auth.openPaths == null ? List.of() : cfg.auth.openPaths;
        for (String p : openPaths) {
            String candidate = normalizePath(p);
            if (pathMatches(requestPath, candidate)) {
                return true;
            }
        }
        return false;
    ...
    private static boolean pathMatches(String requestPath, String openPath) {
        if (requestPath == null || openPath == null) return false;
        if ("/".equals(openPath)) return true;
        if (requestPath.equals(openPath)) return true;
        return requestPath.startsWith(openPath + "/");
    }
