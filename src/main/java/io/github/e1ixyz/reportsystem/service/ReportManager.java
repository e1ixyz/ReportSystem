...
import io.github.e1ixyz.reportsystem.model.ChatMessage;
import io.github.e1ixyz.reportsystem.model.Report;
import io.github.e1ixyz.reportsystem.model.ReportStatus;
import io.github.e1ixyz.reportsystem.model.ReportType;
import io.github.e1ixyz.reportsystem.storage.FileReportStorage;
import io.github.e1ixyz.reportsystem.storage.MysqlReportStorage;
import io.github.e1ixyz.reportsystem.storage.ReportStorage;
import io.github.e1ixyz.reportsystem.storage.StoredReportPayload;
...
public class ReportManager {
    ...
    private Report fromMap(Map<String, Object> m) {
        try {
            ...
            if (r.categoryDisplay == null || r.categoryDisplay.isBlank()) {
                r.categoryDisplay = r.categoryId == null ? "unknown" : r.categoryId;
            }
            r.reason = cleanStr(m.get("reason"));
            r.count = (int) Math.max(1, getLong(m.get("count"), 1));
            r.timestamp = getLong(m.get("timestamp"), System.currentTimeMillis());

            String st = cleanStr(m.get("status"));
    ...
    private String cleanStr(Object o) {
    ...
    private ReportStatus parseStatus(String raw) {
        if (raw == null || raw.isBlank()) return ReportStatus.OPEN;
        try {
            return ReportStatus.valueOf(raw.trim().toUpperCase(Locale.ROOT));
        } catch (IllegalArgumentException ex) {
            log.warn("Unknown report status '{}', defaulting to OPEN.", raw);
